const showPins = (arr) => {
  ...
  arr.forEach((item) => {
    document.querySelector('#content-container').innerHTML += `<div class="card mb-3 border-0 mx-5" style="width: 18rem;">
    <img src="${item.image}" style="height: 211px;"class="card-img-top rounded" alt="${item.title}">
    <div class="card-body">
      <h5 class="card-title">${item.title}</h5>
      <p class="card-text">${item.description}</p>
      <button class="btn btn-outline-dark rounded-pill" data-toggle="modal" data-target="#formModal" id="edit-pin-btn--${item.firebaseKey}">Edit Pin</button>
      <button class="btn btn-outline-dark rounded-pill" id="delete-pin--${item.firebaseKey}--${item.board_id}">Delete Pin</button>
    </div>
  </div>`;
  });
};
