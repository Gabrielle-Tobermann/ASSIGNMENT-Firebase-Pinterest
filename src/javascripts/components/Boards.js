const showBoards = (arr) => {
  ...
  arr.forEach((item) => {
    document.querySelector('#content-container').innerHTML += `<div class="card mb-5 border-0" style="width: 18rem;">
    <img src="${item.image}" style="height: 360px"class="card-img-top" alt="${item.title}">
    <div class="card-body">
    <a href="#"><h5 class="text-dark font-weight-bold" id="board-title--${item.firebaseKey}" class="card-title">${item.title}</h5></a>
    <button class="btn btn-outline-dark rounded-pill font-weight-bold" data-toggle="modal" data-target="#formModal" id="edit-board--${item.firebaseKey}">Edit Board</button>
      <button class="btn btn-outline-dark rounded-pill font-weight-bold" id="delete-board--${item.firebaseKey}">Delete Board</button>
    </div>
  </div>`;
  });
};
